...
func (r *runner) checkAllDisksHealth(ctx context.Context, ssh sshclient.Client, progress stepProgress) error {
	...
	if _, err = tmpFile.WriteString(checkAllDisksScript); err != nil {
	...
	if err != nil {
		return fmt.Errorf("check-all-disks ssh error: %w", err)
	}
	if exitStatus != 0 {
		return fmt.Errorf("check-all-disks failed (exit %d):\n%s", exitStatus, output)
	}
	progress("check-all-disks ok: %s", strings.TrimSpace(output))
	return nil
}
